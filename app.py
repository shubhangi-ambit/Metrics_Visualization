...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Create a replicate Excel workbook with filtered data and the graph
        output = io.BytesIO()
        with xlsxwriter.Workbook(output, {'in_memory': True}) as workbook:
            # Add a worksheet and write the filtered data
            worksheet = workbook.add_worksheet("Company Data")
            for i, col in enumerate(company_data.columns):
                worksheet.write(0, i, col)
            for i, value in enumerate(company_data.values):
                for j, val in enumerate(value):
                    worksheet.write(i + 1, j, val)

            # Calculate the number of rows and columns in the data
            num_rows, num_cols = company_data.shape

            # Insert the saved image to the right of the data
            worksheet.insert_image(num_rows + 2, 0, image_path, {'x_offset': 0, 'y_offset': 0, 'x_scale': 0.5, 'y_scale': 0.5})

        # Allow the user to download the Excel file
        st.markdown("### Download Excel with Filtered Data and Graph")
        st.download_button(label="Download Excel", data=output.getvalue(), file_name="company_metrics_filtered.xlsx", key="download")
